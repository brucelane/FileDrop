...
void FileDropApp::prepare( Settings *settings )
{
	settings->setWindowSize( 1440, 900 );
}
void FileDropApp::fileDrop(FileDropEvent event)
{
	// use the last of the dropped files
	const fs::path &mPath = event.getFile(event.getNumFiles() - 1);
}
void FileDropApp::setup()
{
	mUseBeginEnd = false;
	updateWindowTitle();
	disableFrameRate();
...
void FileDropApp::cleanup()
{
	// save warp settings
	Warp::writeSettings( mWarps, writeFile( mSettings ) );
}

void FileDropApp::update()
{
	// there is nothing to update
}

void FileDropApp::draw()
{
...
void FileDropApp::resize()
{
	// tell the warps our window has been resized, so they properly scale up or down
	Warp::handleResize( mWarps );
}

void FileDropApp::mouseMove( MouseEvent event )
{
...
void FileDropApp::mouseDown( MouseEvent event )
{
	// pass this mouse event to the warp editor first
...
void FileDropApp::mouseDrag( MouseEvent event )
{
	// pass this mouse event to the warp editor first
...
void FileDropApp::mouseUp( MouseEvent event )
{
	// pass this mouse event to the warp editor first
...
void FileDropApp::keyDown( KeyEvent event )
{
...
void FileDropApp::keyUp( KeyEvent event )
{
	// pass this key event to the warp editor first
...
void FileDropApp::updateWindowTitle()
{
	if( mUseBeginEnd )
...
CINDER_APP( FileDropApp, RendererGl( RendererGl::Options().msaa( 8 ) ), &FileDropApp::prepare )
